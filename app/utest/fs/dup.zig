...
test "dup2 succeeds and both fds refer to the same file" {
    ...
    const newfd = 44;
    const ret = linux.dup2(@intCast(fd), newfd);
...
test "dup2 with equal oldfd and newfd is a no-op returning newfd" {
    const fd = linux.open(utest.myname, .{}, 0);
    try testing.expectEqual(.SUCCESS, linux.errno(fd));
    defer _ = linux.close(@intCast(fd));

    const ret = linux.dup2(@intCast(fd), @intCast(fd));
    try testing.expectEqual(.SUCCESS, linux.errno(ret));
    try testing.expectEqual(fd, ret);
}

test "dup2 with an unopened oldfd fails with EBADF" {
    const ret = linux.dup2(999, 50);
    try testing.expectEqual(.BADF, linux.errno(ret));
}

test "dup2 with a negative newfd fails with EBADF" {
    const fd = linux.open(utest.myname, .{}, 0);
    try testing.expectEqual(.SUCCESS, linux.errno(fd));
    defer _ = linux.close(@intCast(fd));

    const ret = linux.dup2(@intCast(fd), -1);
    try testing.expectEqual(.BADF, linux.errno(ret));
}

test "dup2 closes an already-open newfd before reuse" {
    const init = utest.getInit();
    var t = Test.init();
    ...
    const newfd = 45;
    {
        ...
        const ret = linux.dup2(@intCast(content_fd), newfd);
        try testing.expectEqual(.SUCCESS, linux.errno(ret));
    }

    // dup2 onto an already-open newfd must succeed and transparently replace it.
    ...
    const ret = linux.dup2(@intCast(elf_fd), newfd);
    try testing.expectEqual(.SUCCESS, linux.errno(ret));
    defer _ = linux.close(newfd);
