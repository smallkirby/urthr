...
pub fn main(init: std.process.Init) !void {
    ...
    if (args.len != 1 + 1) {
        log.err("Usage: {s} <output.h>", .{args[0]});
    ...
    var writer = output_file.writer(io, &wbuffer);
    const writer_if = &writer.interface;
    defer writer_if.flush() catch {};
    ...
    // Iterate through decls for arch-specific constants.
    try writer_if.writeAll(
        \\
        \\// =============================================================
    ...
    inline for (@typeInfo(arch).@"struct".decls) |field| {
        const name = field.name;
        const value = @field(arch, name);

        try writeItem(value, name, writer_if, "ARCH_");
    }
}
